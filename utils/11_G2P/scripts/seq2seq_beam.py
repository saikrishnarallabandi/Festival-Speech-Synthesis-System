...
class Attention_Batch:
     ...
     def update_minibatch(self):
          ...
          W = 0          
          count = 0
          for sentence_id in self.train_order_e:
              count += 1
              #print count
     ...
     def decode_batch(self, vectors_array, output_array, end_token):
         ...
         #print ones
         last_output_embeddings = lookup_batch(self.output_lookup, ones)
         ...
         # Okay Go on
         words = 0
         for (curr_vec, mask, mat) in zip(isents,masks, vectors_array):
             #print "curr: ", curr_vec
             if first_flag == 0:
                 first_flag = 1
                 last_output_embeddings = lookup_batch(self.output_lookup, ones)
                 ...
                 #bb = dynet.select_cols(vocab_lookup, curr_vec)
                 #bb = dynet.reshape(bb, (1024 ,), len_batch)
                 bb = last_output_embeddings
             ...
             #print y.value()
             output_vector = w * y + b
             err = dynet.pickneglogsoftmax_batch(output_vector, curr_vec)
             #print err.value()
             last_output_embeddings = lookup_batch(self.output_lookup, curr_vec)
             words += len(curr_vec)
             if mask[-1] == 0:
         ...
         #print "Returning"
         return dynet.sum_batches(err_v), words
